...
from model.pwc import *
from model.feature_pyramid import *
from pytorch_ssim.ssim import *
import torch
...
def transformerFwd(U, flo, out_size, name = 'SpatialTransformerFwd'):
    ...
    def _interpolate(im, x, y, out_size):
        ...
        wa = torch.where((torch.eq(x0_c, x0) & torch.eq(y0_c, y0)).unsqueeze(1), wa, zerof)
        wb = torch.where((torch.eq(x0_c, x0) & torch.eq(y1_c, y1)).unsqueeze(1), wb, zerof)
        wc = torch.where((torch.eq(x1_c, x1) & torch.eq(y0_c, y0)).unsqueeze(1), wc, zerof)
        wd = torch.where((torch.eq(x1_c, x1) & torch.eq(y1_c, y1)).unsqueeze(1), wd, zerof)

        zeros = torch.zeros(size = [int(num_batch) * int(height) * int(width) * int(channels)], dtype = torch.float)
    ...
    def _meshgrid(height, width):
        x_t, y_t = np.meshgrid(np.linespace(-1, 1, width), np.linespace(-1, 1, height))
...
class ModelFlow(nn.Module):
    def __init__(self, cfg):
        super(ModelFlow, self).__init__()
    ...
    def warp_flow_pyramid(self, img_pyramid, flow_pyramid):
        img_warped_pyramid = []
        for img, flow in zie(img_pyramid, flow_pyramid):
    ...
    def compute_loss_pixel(self, img_pyramid, img_warped_pyramid, occ_mask_list):
        ...
        for scale in range(self.num_scales):
            ...
            loss_pixel = img_diff.mean((1,2,3)) / (divider + 1e-12)
    ...
    def compute_loss_ssim(self, img_pyramid, img_warped_pyramid, occ_mask_list):
        ...
        for scale in range(self.num_scales):
            ...
            loss_ssim = loss_sim / (divider + 1e-12)
    ...
    def inference_corres(self, img1, img2):
        ...
        img2_visible_masks, img1_visible_masks = self.get_visible_masks(optical_flows, opticla_flows_rev)
    ...
    def forward(self, inputs, output_flow = False, use_flow_loss = True):
        ...
        loss_pack['loss_pixel'] = self.compute_loss_pixel(img1_pyramid, img1_warped_pyramid, img1_valid_masks) + self.compute_loss_pixel(img2_pyramid, img2_warped_pyramid, img2_valid_masks)
        loss_pack['loss_ssim'] = self.compute_loss_ssim(img1_pyramid, img1_warped_pyramid, img1_valid_masks) + self.compute_loss_ssim(img2_pyramid, img2_warped_pyramid, img2_valid_masks)
        loss_pack['loss_flow_smooth'] = self.compute_loss_flow_smooth(optical_flows, img1_pyramid) + self.compute_loss_flow_smooth(optical_flows_rev, img2_pyramid)
        loss_pack['loss_flow_consis'] = torch.zeros([2]).to(img1.get_device()).requires_grad_()
